...
namespace AF {
	...
	public:
		...
		OpenGLTextureCube(const std::string& path, const bool isSRGB = false);
		OpenGLTextureCube(uint32_t rendererID, uint32_t width, uint32_t height);
	...
	private:
		void LoadFromFile(const std::string& path, bool isSRGB);
